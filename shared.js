// ════════════════════════════════════════════════════════════
// 3RA COPA CANTV — Lógica y datos compartidos
// FIFA World Cup 2026 — Grupos oficiales, jugadores, cálculos
// Usado por editor.html para generar results.html y playoffs.html
// ════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════
// GITHUB API — lee y escribe archivos directamente en el repo
// (Contents API: https://docs.github.com/rest/repos/contents)
// ════════════════════════════════════════════════════════════
const GH_API = "https://api.github.com";

// Codificación segura UTF-8 <-> Base64 (soporta emojis/banderas)
...
// Obtiene un archivo del repo (sha + contenido). null si no existe.
async function ghGetFile(cfg, path) {
  const res = await fetch(`${GH_API}/repos/${cfg.owner}/${cfg.repo}/contents/${path}?ref=${cfg.branch}`, {
    headers: {
...
// Crea o reemplaza un archivo en el repo.
...
// Verifica que owner/repo/branch/token son válidos
async function ghTestConnection(cfg) {
  const res = await fetch(`${GH_API}/repos/${cfg.owner}/${cfg.repo}`, {
    headers: { Authorization: `Bearer ${cfg.token}`, Accept: "application/vnd.github+json" },
  });
  if (!res.ok) throw new Error(`No se pudo acceder al repo (${res.status}). Revisa owner/repo/token.`);
  return res.json();
}
